# Advanced Security (CcspAdvSecurity) Agentic Development System

This directory contains the agentic development setup for the CcspAdvSecurity embedded systems project, designed to assist with C/C++/shell development on resource-constrained embedded devices.
...
- **Debug cujo-agent lifecycle issues** using structured triage and RCA
- **Review code** with embedded-safety-aware checklists
- **Manage RFC feature toggles** with correct Init/DeInit patterns
...
├── agents/
│   └── advanced-security.agent.md     # Comprehensive AI agent
├── knowledge/
│   └── reference-data.md              # Enums, syscfg keys, build flags, failure patterns
├── skills/
│   ├── safety-analyzer/
...
**[advanced-security.agent.md](agents/advanced-security.agent.md)** — Comprehensive agent covering:
...
- RFC feature toggle lifecycle management
...
| **Shell Scripts** | `**/*.sh` | POSIX compliance, error handling, cujo-agent lifecycle |
| **Build System** | `Makefile.am`, `configure.ac` | Autotools, cross-compilation |

### 5. Knowledge Base

**[reference-data.md](knowledge/reference-data.md)** — Feature enums, syscfg keys, DML handler mappings, Init/DeInit function names, script flags, build flags, constants, failure patterns, thread safety notes.

## Best Practices

### Memory Management

- Check all `AnscAllocateMemory()` / `malloc()` return values
- Use single exit point with `goto cleanup`
- `snprintf` instead of `sprintf`/`strcpy`
- NULL pointer after `AnscFreeMemory()` / `free()` in long-lived contexts
- Run valgrind on all tests

### Thread Safety

- All shared data under `logMutex` or atomics
- No blocking I/O while holding locks
- `while` loop for `pthread_cond_wait`
- Logger thread uses `pthread_cond_timedwait` with `logMutex`/`logCond`
- Sysevent handler thread is long-lived — guard with shutdown checks

### Platform Independence

- Use `stdint.h` types (`uint32_t`, `int16_t`)
- Platform-specific headers behind `#ifdef` guards (`_COSA_BCM_MIPS_`, `_COSA_INTEL_XB3_ARM_`, `CONFIG_CISCO`)
- Test cross-compilation on ARM and MIPS

### Security

- All URL/string inputs validated via `isValidUrl()` before shell invocation
- Command injection prevention: check for `;`, `&`, `|`, `'` in parameters
- Use `v_secure_system()` instead of `system()` for all script calls
- Never log sensitive subscriber data at INFO level
