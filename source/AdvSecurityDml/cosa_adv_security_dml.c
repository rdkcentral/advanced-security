    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        *pBool = g_pAdvSecAgent->pAdvSecCPEProtectionNetwork_RFC->bEnable;
        return TRUE;
    }
    CcspTraceWarning(("Unsupported parameter '%s'\n", ParamName));
    return FALSE;
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        if(bValue == g_pAdvSecAgent->pAdvSecCPEProtectionNetwork_RFC->bEnable)
                return TRUE;
        if( bValue )
                returnStatus = CosaAdvSecCPEProtectionNetworkInit(g_pAdvSecAgent->pAdvSecCPEProtectionNetwork_RFC);
        else
                returnStatus = CosaAdvSecCPEProtectionNetworkDeInit(g_pAdvSecAgent->pAdvSecCPEProtectionNetwork_RFC);
