    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        *pBool = g_pAdvSecAgent->pDFLastSeen_RFC->bEnable;
        return TRUE;
    }
    CcspTraceWarning(("Unsupported parameter '%s'\n", ParamName));
    return FALSE;
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        if(bValue == g_pAdvSecAgent->pDFLastSeen_RFC->bEnable)
                return TRUE;
        if( bValue )
                returnStatus = CosaAdvSecDFLastSeenInit(g_pAdvSecAgent->pDFLastSeen_RFC);
        else
                returnStatus = CosaAdvSecDFLastSeenDeInit(g_pAdvSecAgent->pDFLastSeen_RFC);
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        *pBool = g_pAdvSecAgent->pDFDNSSafeSearch_RFC->bEnable;
        return TRUE;
    }
    CcspTraceWarning(("Unsupported parameter '%s'\n", ParamName));
    return FALSE;
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        if(bValue == g_pAdvSecAgent->pDFDNSSafeSearch_RFC->bEnable)
                return TRUE;
        if( bValue )
                returnStatus = CosaAdvSecDFDNSSafeSearchInit(g_pAdvSecAgent->pDFDNSSafeSearch_RFC);
        else
                returnStatus = CosaAdvSecDFDNSSafeSearchDeInit(g_pAdvSecAgent->pDFDNSSafeSearch_RFC);
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        *pBool = g_pAdvSecAgent->pDFLocalHTTPUA_RFC->bEnable;
        return TRUE;
    }
    CcspTraceWarning(("Unsupported parameter '%s'\n", ParamName));
    return FALSE;
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        if(bValue == g_pAdvSecAgent->pDFLocalHTTPUA_RFC->bEnable)
                return TRUE;
        if( bValue )
                returnStatus = CosaAdvSecDFLocalHTTPUAInit(g_pAdvSecAgent->pDFLocalHTTPUA_RFC);
        else
                returnStatus = CosaAdvSecDFLocalHTTPUADeInit(g_pAdvSecAgent->pDFLocalHTTPUA_RFC);
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        *pBool = g_pAdvSecAgent->pCpeProtectionNetwork_RFC->bEnable;
        return TRUE;
    }
    CcspTraceWarning(("Unsupported parameter '%s'\n", ParamName));
    return FALSE;
    ...
    )
{
    ...
    if( AnscEqualString(ParamName, "Enable", TRUE))
    {
        if(bValue == g_pAdvSecAgent->pCpeProtectionNetwork_RFC->bEnable)
                return TRUE;
        if( bValue )
                returnStatus = CosaCpeProtectionNetworkInit(g_pAdvSecAgent->pCpeProtectionNetwork_RFC);
        else
                returnStatus = CosaCpeProtectionNetworkDeInit(g_pAdvSecAgent->pCpeProtectionNetwork_RFC);
