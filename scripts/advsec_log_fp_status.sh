...
check_status()
{
    ...
    if [ -e ${ADVSEC_DEVICEFINGERPRINTLASTSEEN_ENABLED_PATH} ]; then
        print_telemetry_log ${ADV_DEVICEFINGERPRINTLASTSEEN_RFC_ENABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    else
        print_telemetry_log ${ADV_DEVICEFINGERPRINTLASTSEEN_RFC_DISABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    fi

    if [ -e ${ADVSEC_DFDNSSAFESEARCH_ENABLED_PATH} ]; then
        print_telemetry_log ${ADV_DFDNSSAFESEARCH_RFC_ENABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    else
        print_telemetry_log ${ADV_DFDNSSAFESEARCH_RFC_DISABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    fi

    if [ -e ${ADVSEC_DEVICEFINGERPRINTLOCALHTTPUA_ENABLED_PATH} ]; then
        print_telemetry_log ${ADV_DEVICEFINGERPRINTLOCALHTTPUA_RFC_ENABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    else
        print_telemetry_log ${ADV_DEVICEFINGERPRINTLOCALHTTPUA_RFC_DISABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    fi

    if [ -e ${ADVSEC_CPEPROTECTIONNETWORK_ENABLED_PATH} ]; then
        print_telemetry_log ${ADV_CPEPROTECTIONNETWORK_RFC_ENABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    else
        print_telemetry_log ${ADV_CPEPROTECTIONNETWORK_RFC_DISABLE_LOG} ${ADVSEC_AGENT_LOG_PATH}
    fi
